...
packs <- c('dplyr','mcmcplots','runjags','rjags','reshape2','raster','sp','rgdal',
           'parallel', 'stringr', 'foreach','doParallel', "lubridate", "coda",
           'rgeos', 'adehabitatHR','geometry', 'vioplot', 'scales', 'grDevices')

package_load(packs)



# modified version of violin plots from Mason
my_vioplot <- function (x, ..., range = 1.5, h = NULL, ylim = NULL, names = NULL, 
                        horizontal = FALSE, col = "magenta", border = "black", lty = 1, 
                        lwd = 1, rectCol = "black", colMed = "white", pchMed = 19, 
                        at, add = FALSE, wex = 1, drawRect = TRUE, side="both", lwdMed = 1) 
{
  datas <- list(x, ...)
  n <- length(datas)
  if (missing(at)) 
  ...
  if (!horizontal) {
    ...
    for (i in 1:n) {
      ...
      if (drawRect) {
        if(side == "right"){
          ...
          #        q3[i], col = rectCol)
          # median line segment
          # lines(x = c(at[i] - radj*(h+0.5),
          #             at[i],
          #             at[i] + ladj*(h+0.5)),
          #       y = rep(med[i],3), col=colMed, lwd=lwdMed)
           points(y = at[i], x = med[i], pch=pchMed, col = colMed, cex = 3)
        }
        if(side == "left"){
          lines(at[c(i, i)], c(lower[i], upper[i]), lwd = lwd, 
                lty = lty)
    ...
    for (i in 1:n) {
      ...
      if (drawRect) {
        ...
        # lines(y = c(at[i] - radj*(h+0.5),
        #             at[i],
        #             at[i] + ladj*(h+0.5)),
        #       x = rep(med[i],3), lwd = lwdMed, col=colMed)
        points(y = at[i], x = med[i], pch=pchMed, col = colMed, cex = 3)
      }
    }
  }
